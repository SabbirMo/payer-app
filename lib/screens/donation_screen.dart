...
class DonationScreen extends StatelessWidget {
  ...
  static const String paypalLink =
      'https://www.paypal.com/paypalme/PayPal2kery';
  ...
  Future<void> _launchPayPalURL(String url) async {
    ...
    try {
      // Try to open in the native PayPal app first
      launched = await launchUrl(
        uri,
        mode: LaunchMode.externalNonBrowserApplication,
      );
    } catch (e) {
      // If the PayPal app is not found, launchUrl throws a PlatformException
      launched = false;
    }

    if (!launched) {
      log('paypal not exist');
      try {
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border:
                    Border.all(color: const Color(0xFFD4A017).withOpacity(0.4)),
              ),
              child: Column(
                children: [
            ...
            _buildPaymentCard(
              ...
              details: [
                _buildDetailRow(Icons.email_rounded, 'Email', paypalEmail,
                    () => _copyToClipboard(context, paypalEmail)),
                _buildDetailRow(
                    Icons.link_rounded,
                    'Link',
                    'paypal.me/PayPal2kery',
                    () => _launchPayPalURL(paypalLink)),
                const SizedBox(height: 16),
                Text('Select Amount:',
                    style: GoogleFonts.cinzel(
                        color: Colors.white70,
                ...
                Row(
                  ...
                  children: [
                    ...
                    _buildAmountButton(context, 20),
                    _buildAmountButton(context, 50),
                    _buildAmountButton(context, 100),
                  ],
                ),
              ],
              onTap: () => _launchPayPalURL(paypalLink),
              buttonText: 'Custom Amount',
            ),
    ...
    return Container(
      decoration: BoxDecoration(
        gradient: LinearGradient(
            colors: [color.withOpacity(0.2), const Color(0xFF0F0F20)]),
  ...
  Widget _buildAmountButton(BuildContext context, int amount) {
    return GestureDetector(
      onTap: () => _launchPayPalURL('$paypalLink/$amount'),
      child: Container(
  ...
  Widget _buildDetailRow(
      IconData icon, String label, String value, VoidCallback? onCopy) {
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Text('$label: ',
              style: GoogleFonts.cinzel(color: Colors.white54, fontSize: 11)),
          Expanded(
              child: Text(value,
                  style: GoogleFonts.cinzel(color: Colors.white, fontSize: 11),
